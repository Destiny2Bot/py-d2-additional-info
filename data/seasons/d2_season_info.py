...
def getCurrentSeason() -> int:
    ...
    for i in range(len(D2SeasonEnum), 0, -1):
        seasonDate = datetime.strptime(
            f"{D2SeasonInfo[i].releaseDate} {D2SeasonInfo[i].resetTime}",
            "%Y-%m-%d %H:%M:%S%z",
        )
        ...
        closeToNewSeason = (
            isToday(seasonDate)
            and numHoursBetween(today, seasonDate) < CLOSE_TO_RESET_HOURS
        )
...
def isToday(someDate: datetime):
    today = datetime.now()
    return (
        today.year == someDate.year
        and today.month == someDate.month
        and today.day == someDate.day
    )
