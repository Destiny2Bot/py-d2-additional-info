...
def convertMixedStringToLeadingCapCamelCase(input: str) -> str:
    input = input.replace(' ', '-').replace(':', '-').replace('/', '-').replace('.', '-').replace('_', '-').replace('#', '-').replace('：', '-')
    return ''.join(k.lower().capitalize() for k in input.split('-'))


for item in inventoryItems:
    if item.get('plug') and not item.get('redacted'):
        identifier = convertMixedStringToLeadingCapCamelCase(item['plug']['plugCategoryIdentifier'])
...
def tryToGetAdditionalStringContent(thing: dict) -> str:
    ...
    if thing in allSocketCategories:
        ...
        exampleItems = [i for i in inventoryItems if i.get('sockets') and 
        [j for j in i['sockets']['socketCategories'] if j['socketCategoryHash'] == thing['hash']]]

        if not exampleItems:
            labels.append('UNUSED')
        else:
            itemTypes = set([i.get('itemTypeDisplayName') for i in exampleItems if i.get('itemTypeDisplayName')])

            # 仅当所有找到的项目具有相同的项目类型时才使用此标签
            if len(itemTypes) == 1:
                labels.append(convertMixedStringToLeadingCapCamelCase(list(itemTypes)[0]))
...
for i in enumSources:
    ...
    for j in i['data']:
        if j.get('redacted') or not j['displayProperties'].get('name') or j['hash'] in ignoreHashes:
            continue
        identifier = convertMixedStringToLeadingCapCamelCase(j['displayProperties']['name'])
    ...
    for j in i['data']:
        ...
        if generatedEnums[i['name']].get(identifier) and generatedEnums[i['name']].get(identifier) != j['hash']:
    ...
    # 现在处理重复值
    deDupedIdentifiers = [convertMixedStringToLeadingCapCamelCase(dupeNamedItem['displayProperties']['name']) 
    + tryToGetAdditionalStringContent(dupeNamedItem) for dupeNamedItem in dupeNamedItems]
    ...
    for dupeNamedItem in dupeNamedItems:
        identifier = convertMixedStringToLeadingCapCamelCase(dupeNamedItem
        ['displayProperties']['name']) + tryToGetAdditionalStringContent(dupeNamedItem)
