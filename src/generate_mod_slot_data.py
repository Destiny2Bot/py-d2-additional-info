...
from tools.JsonIO import writejson
...
def getCompatibleTags(exampleArmorSocketEntry: dict):
    ...
    if temp:
        ...
        if temp:
            return deduplication([seasonTagFromMod(get('DestinyInventoryItemDefinition', plugItem['plugItemHash'])) 
            for plugItem in temp['reusablePlugItems'] if get('DestinyInventoryItemDefinition', plugItem.get('plugItemHash'))])
...
def getEmptySeasonalModSocketsInfo(emptyModSocket: dict):
    ...
    # 与模组插槽相关的赛季编号
    season = seasonNumberByPlugCategoryIdentifier[emptyModSocket['plug']['plugCategoryIdentifier']]
    ...
    plugCategoryHashes = deduplication([item['plug']['plugCategoryHash'] for item in inventoryItems 
    if item.get('itemTypeDisplayName') == itemTypeDisplayName if item.get('plug') and item['plug']['plugCategoryHash']])

    # 此 SocketType 支持的 plugCategoryHashes
    compatiblePlugCategoryHashes = [plugType['categoryHash'] for plugType in get(
...
def findExampleSocketByEmptyModHash(emptyModSocketHash: int):
    targetInventoryItems = [i for i in inventoryItems if i.get('sockets') and any([j for j in i['sockets']['socketEntries'] if j['singleInitialItemHash'] == emptyModSocketHash])]
    if targetInventoryItems:
        return [j for j in targetInventoryItems[0]['sockets']['socketEntries'] if j['singleInitialItemHash'] == emptyModSocketHash][0]
    else:
        return None

# 查找名称为空模组插槽的插槽信息
emptySeasonalModSockets = [i for i in inventoryItems if i['displayProperties']['name'] == '空模组插槽' and
i.get('plug') and i['plug']['plugCategoryIdentifier'].startswith(seasonalPlugCategoryIdentifier)]

# 按 hash 对空模组插槽排序
emptySeasonalModSockets.sort(key = lambda x: x['hash'])

modMetadatas: List[ModSocketMetadata] = [i for i in [getEmptySeasonalModSocketsInfo(emptyModSocket) for emptyModSocket in emptySeasonalModSockets] if i]
...
import ujson
modMetadatasStr = ujson.dumps([i.dict() for i in modMetadatas], ensure_ascii = False, indent = 4)
